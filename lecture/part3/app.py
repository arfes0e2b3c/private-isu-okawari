...
def post_login():
    ...
    user = try_login(
        flask.request.form["account_name"], flask.request.form["password"])
    if user:
...
def post_register():
    ...
    cursor = db().cursor()
    cursor.execute(
        "SELECT 1 FROM users WHERE `account_name` = %s", (account_name,))
    ...
    cursor.execute(
        query, (account_name, calculate_passhash(account_name, password)))
...
def get_user_list(account_name):
    ...
    cursor.execute(
        "SELECT `id` FROM `posts` WHERE `user_id` = %s", (user["id"],))
